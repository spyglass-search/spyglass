...
"#.into()
    ...
    if let Some(ctxt) = doc_context {
        start.push_str("\n### Input:\n");
        start.push_str(&ctxt.iter().map(|x| x.trim()).collect::<Vec<_>>().join(" "));
        start.push('\n');
    }
...
fn run_model(
    ...
    tokio::spawn(async move {
        let _ = tx.send(ChatUpdate::LoadingPrompt);
    });
        ...
        move |t| {
            ...
            match t {
                OutputToken::Token(c) => {
                    ...
                    if output_prompt {
                        tokio::spawn(async move {
                            let _ = tx.send(ChatUpdate::Token(c.to_string()));
                        });
                ...
                OutputToken::EndOfText => {
                    let tx = tx.clone();
                    tokio::spawn(async move {
                        let _ = tx.send(ChatUpdate::EndOfText);
                    });
                }
            }
            Ok(())
    ...
    match res {
        ...
        Err(llama_rs::InferenceError::ContextFull) => {
            let _ = stream.send(ChatUpdate::Error(
        ...
        let recv_task = tokio::spawn(async move {
            ...
            while let Some(msg) = rx.recv().await {
                match msg {
                    ...
                    ChatUpdate::Error(msg) => {
                        eprintln!("Received an error: {}", msg);
                        break;
                    }
                    ChatUpdate::EndOfText => break,
                    _ => {}
                }
            }
        });
